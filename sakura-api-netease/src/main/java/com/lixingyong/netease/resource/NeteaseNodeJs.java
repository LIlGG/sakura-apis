package com.lixingyong.netease.resource;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import com.lixingyong.netease.utils.NeteaseException;
import com.lixingyong.netease.resource.constant.NeteaseNodeApiConstant;
...
import com.lixingyong.netease.resource.model.params.ArtistSongsParam;
import com.lixingyong.netease.resource.model.params.HighQualityParam;
import com.lixingyong.netease.resource.model.params.SearchParam;
import com.lixingyong.netease.resource.model.po.HighQualityResult;
import com.lixingyong.netease.resource.model.po.LyricResult;
import com.lixingyong.netease.resource.model.po.MusicResult;
import com.lixingyong.netease.resource.model.po.PlayListResult;
import com.lixingyong.netease.resource.model.po.SearchListResult;
import com.lixingyong.netease.resource.model.po.SongResult;
import java.net.URI;
...
public class NeteaseNodeJs {
    ...
    public PlayList playListDetail(long id) {
        ...
        if (Objects.isNull(playListResult)) {
            throw new NeteaseException("未查询到歌单列表");
        }
        if (playListResult.getCode() == NeteaseCodeStatus.OK) {
            return playListResult.getPlaylist();
        }
        throw new NeteaseException(playListResult.getCode().getMsg());
    }
     ...
     * </ul>
     *
     * <p>搜索支持列表参见 {@link com.lixingyong.netease.resource.model.enums.SearchType}</p>
     *
     * @see SongSearchList
     * @see AlbumSearchList
    ...
    public SearchList search(SearchParam searchParam) {
        ...
        if (Objects.isNull(searchListResult)) {
            throw new NeteaseException("网易云音乐搜索失败，未获取到数据");
        }
        if (searchListResult.getCode() != NeteaseCodeStatus.OK) {
            throw new NeteaseException(searchListResult.getCode().getMsg());
        }
        ...
        if (Objects.isNull(searchList)) {
            throw new NeteaseException("搜索列表为空");
        }
        return searchList;
    }
    ...
    public List<PlayList> highQuality(HighQualityParam highQualityParam) {
        ...
        if (Objects.isNull(highQualityResult)) {
            throw new NeteaseException("精品歌单获取失败");
        }
        if (highQualityResult.getCode() != NeteaseCodeStatus.OK) {
            throw new NeteaseException(highQualityResult.getCode().getMsg());
        }
    ...
    public List<Song> artistSongs(ArtistSongsParam artistSongsParam) {
        ...
        if (Objects.isNull(songResult)) {
            throw new NeteaseException("艺术家" + artistSongsParam.getId() + "的歌曲获取失败");
        }
        if (songResult.getCode() != NeteaseCodeStatus.OK) {
            throw new NeteaseException(songResult.getCode().getMsg());
        }
    ...
    public List<Song> songByMusicId(Collection<Long> trackIds) {
        ...
        SongResult songResult = JSONObject.parseObject(response.body(), SongResult.class);
        if (Objects.isNull(songResult)) {
            throw new NeteaseException("歌曲详情获取失败");
        }
        if (songResult.getCode() != NeteaseCodeStatus.OK) {
            throw new NeteaseException(songResult.getCode().getMsg());
        }
    ...
    public Lyric getLyric(Long id) {
        ...
        if (lyricResult.getCode() != NeteaseCodeStatus.OK) {
            throw new NeteaseException(lyricResult.getCode().getMsg());
        }
